<template>
  <div class="page">
    ...
    <div class="page-header">
      <div>
        <div class="page-title">Bank Nifty Dashboard</div>
        <div class="page-subtitle">BANK NIFTY Options · {{ strategySubtitle }} · 5-min candles</div>
      </div>
    ...
    <div v-if="showData" class="mstrip">
      ...
      <div class="mtile">
        <div class="lbl">BankNifty Futures</div>
    ...
    <div class="card chart-card">
      ...
      <div v-if="!todayCandles.length" class="chart-empty">
        Start the BankNifty engine to see the live candlestick chart with VWAP &amp; EMA overlays.
      </div>
    ...
    <div class="card">
      <div class="card-header">
        <div class="card-title">Paper Trade Log — BANKNIFTY</div>
        <div style="display:flex;gap:8px;align-items:center">
          <button class="btn s-btn" style="padding:5px 12px;font-size:11px" @click="loadTrades">↻ Refresh</button>
          <a href="/auto-trading/banknifty/paper-log/download" class="btn s-btn" style="padding:5px 12px;font-size:11px">⬇ CSV</a>
        </div>
      </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>P&amp;L ₹</th><th>P&amp;L %</th><th>Exit Reason</th><th>Entry Reason</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!trades.length">
              <td colspan="12" class="empty-state">No BANKNIFTY paper trades yet. Start the engine to begin.</td>
            </tr>
            <tr v-for="t in [...trades].reverse()" :key="t.trade_number">
              <td style="color:var(--text-muted)">{{ t.trade_number }}</td>
              <td>{{ t.date }}</td>
              <td><span class="pill" :class="t.option_type === 'CE' ? 'pill-ce' : 'pill-pe'">{{ t.option_type }}</span></td>
    ...
    <div class="card">
      <div class="card-header">
        <div class="card-title">Candle Log Export — BANKNIFTY</div>
      ...
      <div style="font-size:11px;color:var(--text-muted)">Select a date to download the BANKNIFTY 5-min candle log with all indicator snapshots.</div>
    </div>
...
const strategyNote = computed(() => {
  ...
  if (m === 'trend_pullback') {
    return `Trend Pullback · 1-strike-ITM · Spot SL 0.25% · Partial book +7%/+14% · Trail 4% · Max ${mt}/day · Force exit ${ft} · Lot size 30`
  }
  if (m === 'mean_reversion') {
    return `Mean Reversion · Fade failed-spike · 1-strike-ITM · Spot SL 0.35% · Partial book +7%/+14% · Hold ≤ 90 min · Max ${mt}/day · Force exit ${ft} · Lot size 30`
  }
  return `VWAP + EMA(20) · Dynamic SL 12–22% · Trailing SL from +20% · RSI + Volume filters · Max ${mt}/day · Force exit ${ft} · Lot size 30`
})
...
async function refreshAll() {
  try {
    const [sRes, cRes] = await Promise.all([
      ...
      fetch('/auto-trading/banknifty/candles'),
    ])
...
async function loadTrades() {
  try {
    const r = await fetch('/auto-trading/banknifty/paper-log')
...
async function startEngine() {
  loading.value = true
  setMsg('Starting BankNifty engine…', '')
  try {
    const r = await fetch('/auto-trading/banknifty/start', { method: 'POST' })
    const d = await r.json()
    if (!r.ok) { setMsg(d.detail || 'Error', 'err'); return }
    setMsg('BankNifty engine started (' + d.mode + '). CE: ' + d.instruments.ce + ' | PE: ' + d.instruments.pe, 'ok')
    await refreshAll()
...
async function stopEngine() {
  loading.value = true
  setMsg('Stopping BankNifty…', '')
  try {
    const r = await fetch('/auto-trading/banknifty/stop', { method: 'POST' })
    const d = await r.json()
    if (!r.ok) { setMsg(d.detail || 'Error', 'err'); return }
...
function downloadCandleLog() {
  ...
  window.location.href = '/auto-trading/banknifty/candle-log/download/' + candleLogDate.value
}
